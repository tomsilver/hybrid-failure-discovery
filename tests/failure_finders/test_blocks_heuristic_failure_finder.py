...
This test suite validates the HeuristicFailureFinder's ability to
discover failures in the Blocks environment using a heuristic that
encourages failure-prone scenarios (e.g., blocks close together, complex
configurations).
"""
...
def test_heuristic_failure_finder_blocks():
    ...
    def blocks_heuristic(traj):
        ...
        complexity_score = 1.0 - min(
            1.0, num_blocks / 8.0
        )  # More aggressive: favor 8+ blocks
        ...
        tower_score = 1.0 - min(
            1.0, height_range / 0.5
        )  # Favor towers > 0.25 units tall
        ...
        combined_score = (
            distance_score * 0.5 + complexity_score * 0.3 + tower_score * 0.2
        )
