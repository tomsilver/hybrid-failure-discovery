"""Blackbox optimizers for finding failures."""
...
class AnnealingFailureFinder(Annealer, FailureFinder):
    ...
    def __init__(
        ...
        state = self.random_run(
            env, controller
        )  # input signal; named state in annealer
    ...
    def move(self):
        ...
        idx = np.random.randint(
            0, self._max_trajectory_length
        )  # Choose a random time index to modify
    ...
    def select_state(self, next_states, input):
        """Select the next state that matches the input signal."""
        gx, gy = input
    ...
    #                 next_states, input
    #             )  # Select next state that matches the input signal
    #         except:
    #             st()
