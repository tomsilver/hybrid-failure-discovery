...
#     init_velocities: tuple[float, ...] = (1.0,) * 6
#     init_positions: tuple[float, ...] = (0.5, 1.25, 2.0, 2.75, 3.5, 4.25)
#     dt: float = 0.01
#     belt_length: float = 5.0
#     box_width: float = 0.4
#     box_height: float = 0.5
#     gravity: float = 9.81
#     drop_start_height: float = 1.0  # height from which boxes start dropping
#     min_spacing: float = 0.72       # still used in test checks
...
#         # parameters for periodic dropping
#         self._next_drop_step = 0
#         self._drop_interval = 20  # average steps between drops
...
#         # Predict new positions (only if on belt)
#         predicted_positions = []
#         for p, v, h in zip(positions, velocities, falling_heights, strict=True):
#             if h > 0.0:
#                 predicted_positions.append(p)  # falling boxes don't move horizontally
...
#         # Periodic auto-drop logic
#         if self.auto_drop:
#             if self._step_count >= self._next_drop_step:
#                 # schedule next drop with jitter
#                 jitter = self._rng.integers(-5, 6)
#                 self._next_drop_step = self._step_count + max(
#                     5, self._drop_interval + jitter
#                 )
#                 # drop new box at left edge, above belt
#                 positions.append(0.0)
#                 velocities.append(new_speed)
#                 falling_heights.append(drop_start_height)
#                 predicted_positions.append(0.0)

#         # Remove boxes off either end of belt
#         keep_pos, keep_vel, keep_fall = [], [], []
#         for p, v, h in zip(predicted_positions, velocities, falling_heights):
#             if (p >= belt_length - box_width) or (p < 0):
#                 continue
...
#             rect = plt.Rectangle(
#                 (pos, 0.7 + height),
#                 self.scene_spec.box_width,
#                 self.scene_spec.box_height,
#                 color="blue",
#             )
#             ax.add_patch(rect)
...
class ConveyorBeltSceneSpec:
    ...
    min_spacing: float = 0.72


class ConveyorBeltEnv(ConstraintBasedGymEnv[ConveyorBeltState, ConveyorBeltAction]):
    """1D conveyor belt environment with external control for autodrop."""
    ...
    def __init__(
        ...
        scene_spec: ConveyorBeltSceneSpec = ConveyorBeltSceneSpec(),
        seed: int = 0,
        autodrop_interval: int = 10,  # steps between automatic drops
    ):
        self.scene_spec = scene_spec
        self.render_mode = render_mode
        self.observation_dim = 6
        ...
        # External autodrop control
        self._autodrop_active = False
        self._autodrop_interval = autodrop_interval
        self._last_drop_step = 0

        super().__init__(seed)

    # --- External control API ---
    def manual_drop(self):
        """Enable continuous autodrop at fixed rate."""
        self._autodrop_active = True
        self._last_drop_step = self._step_count
    ...
    def _create_action_space(self) -> FunctionalSpace[ConveyorBeltAction]:
        return FunctionalSpace(
            contains_fn=lambda x: isinstance(x, ConveyorBeltAction),
            sample_fn=lambda rng: ConveyorBeltAction(index=rng.integers(0, self.action_dim)),
        )
    ...
    def get_initial_states(self) -> EnumSpace[ConveyorBeltState]:
        return EnumSpace(
            [
                ConveyorBeltState(
                    positions=np.array(self.scene_spec.init_positions, dtype=np.float32),
                    velocities=np.array(self.scene_spec.init_velocities, dtype=np.float32),
                    falling_heights=np.zeros(self.box_count, dtype=np.float32),
                )
            ]
        )

    def get_next_states(self, state: ConveyorBeltState, action: ConveyorBeltAction) -> EnumSpace[ConveyorBeltState]:
        dt = self.scene_spec.dt
        gravity = self.scene_spec.gravity
        belt_length = self.scene_spec.belt_length
        box_width = self.scene_spec.box_width
        ...
        # --- External autodrop ---
        if self._autodrop_active:
            if self._step_count - self._last_drop_step >= self._autodrop_interval:
                # drop new box if spacing allows
                candidate_positions = predicted_positions + [0.0]
                candidate_positions_sorted = sorted(candidate_positions)
                diffs = np.diff(candidate_positions_sorted)
                tolerance = 0.02
    ...
    def actions_are_equal(self, action1: ConveyorBeltAction, action2: ConveyorBeltAction) -> bool:
        return action1.index == action2.index
